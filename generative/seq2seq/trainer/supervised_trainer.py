    ...
    def _train_epoches(self, data, model, n_epochs, start_epoch, start_step,
        ...
        for epoch in range(start_epoch, n_epochs + 1):
            ...
            for batch in batch_generator:
                ...
                target_variables, _ = getattr(batch, seq2seq.tgt_field_name)
                spk_inputs = [getattr(batch, str(i)) for i in range(self.args.num_sentence)]

                loss = self._train_batch(spk_inputs, input_variables, input_lengths.tolist(), target_variables, model, teacher_forcing_ratio)
